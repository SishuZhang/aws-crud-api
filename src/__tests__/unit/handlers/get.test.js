...
const { handler } = require('../../../handlers/get');
...
describe('Get Handler', () => {
  ...
  it('should get an order by ID successfully', async () => {
    ...
    expect(mockDynamoDB.get).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should handle DynamoDB errors', async () => {
    // Mock DynamoDB error
    mockDynamoDB.get().promise.mockRejectedValueOnce(new Error('DynamoDB error'));
    ...
    const body = JSON.parse(response.body);
    expect(body.message).toBe('Error retrieving order');
